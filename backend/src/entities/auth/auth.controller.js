...
class AuthController {
   ...
   * Only allows login if the user has admin privileges.
   *
   * @param {Object} req - Express request object
   * @param {Object} req.body - Request body containing login credentials
   * @param {string} req.body.username - Username or email for login
   * @param {string} req.body.password - User's password
   * @param {Object} res - Express response object
   * @returns {Object} JSON response with admin user data and session info
   */
   ...
   *
   * Retrieves the current admin user's session data if authenticated and admin.
   *
   * @param {Object} req - Express request object with session data
   * @param {Object} res - Express response object
   * @returns {Object} JSON response with current admin data or error
   */
  static async getAdminSession(req, res) {
    try {
      // Check if user is authenticated
      if (!req.session.userId) {
        return res.status(401).json({ error: "Not authenticated" });
